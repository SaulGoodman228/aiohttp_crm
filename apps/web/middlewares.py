...
from apps.web.utils import error_json_response
...
async def error_handling_middleware(request,handler):
    try:
        ...
        return error_json_response(http_status=400, status='bad request', message='invalid data', data=json.loads(e.text))
...
#Подключает в наш проект middleware
